...
function Gfn_ResizeInitSD(nWidth,nHeight)
{
	this.OnSize = "Gfn_frm_OnSizeSD";
	...
	for ( var i = 0 ; i < this.Components.Count ; i++ )
	{
		...
		{
			Gfn_SubFormResizeInitSD(Components[i]);
		}
	}
	fv_FirstResize = true;
	
	Gfn_frm_OnSizeSD(this,this.width,this.height);
}


function Gfn_SubFormResizeInitSD(obj)
{
	var seq = 0;
	...
	for ( var i = 0 ; i < obj.Components.Count ; i++ )
	{
		...
		{
			Gfn_SubFormResizeInitSD(obj.Components[i]);
		}
	}
...
function Gfn_ResizeProcSD(nCx,nCy)
{
	fv_SubSeq = -1;
	...
	for ( var i = 0 ; i < this.Components.Count ; i++ )
	{
		...
		else if (this.Components[i].GetType() == "Div")
		{
			...
			// GRY : 추가 : 2012.09.03
			if ( 
					// substr(this.Components[i].ID, 0, 8) == "div_buyr" ||	// 거래선	
					// substr(this.Components[i].ID, 0, 8) == "div_sman" ||	// 담당자
					// substr(this.Components[i].ID, 0, 8) == "div_vbvg" ||	// 부서/팀
					// substr(this.Components[i].ID, 0, 8) == "div_lifn" ||	// 협력업체
					// substr(this.Components[i].ID, 0, 8) == "div_prjt" ||	// 프로젝트
					// substr(this.Components[i].ID, 0, 8) == "div_sale" ||	// SO
					// substr(this.Components[i].ID, 0, 8) == "div_frbu" ||	//
					// substr(this.Components[i].ID, 0, 8) == "div_tobu" ||	//
					// substr(this.Components[i].ID, 0, 8) == "div_frli" ||	//
					// substr(this.Components[i].ID, 0, 8) == "div_toli" ||	//
					// substr(this.Components[i].ID, 0, 8) == "div_frsm" ||	//
					// substr(this.Components[i].ID, 0, 8) == "div_mana" ||	// 영업담당자
					// substr(this.Components[i].ID, 0, 8) == "div_plan" ||	// 년월
					// substr(this.Components[i].ID, 0, 8) == "div_tosm"		//
					 substr(this.Components[i].ID, 0, 4) == "div_"		//
		...
		{
			// form안에 컴포넌트 처리
			if ( this.Components[i].userdata != 'fix')
			{
				bProcSizeFlag = true;
				bX = true;
				if ( this.Components[i].userdata != 'fix2' )
				{
					// y좌표 고정
					bY = true;
				}
		...
		else if ( this.Components[i].GetType() == "Tab" ) 
		{
		    //---------------------------------------------------------------
			// GRY : 추가 : 2012.09.03
			if (substr(this.Components[i].ID, 0, 8) == "tab_hdel")
			{
				// 리사이즈 제외 
				bProcSizeFlag = false;
			}
...
//		trace(this.Components[i].ID + "[" + this.Components[i].GetType() + "] : " + this.Components[i].Width);
		...
		{
			Gfn_SubFormResizeProcSD(this.Components[i]);
		}
	}
}


function Gfn_SubFormResizeProcSD(obj)
{
	var nCx = obj.width;
	var nCy = obj.height;
	var seq = 0;
	...
	for ( var i = 0 ; i < loopCnt ; i++ )
	{
		...
		else if ( obj.Components[i].GetType() == "Div" )
		{
			...
			// GRY : 추가 : 2012.09.03
			if ( 
					// substr(obj.Components[i].ID, 0, 8) == "div_buyr" ||	// 거래선	
					// substr(obj.Components[i].ID, 0, 8) == "div_sman" ||	// 담당자
					// substr(obj.Components[i].ID, 0, 8) == "div_vbvg" ||	// 부서/팀
					// substr(obj.Components[i].ID, 0, 8) == "div_lifn" ||	// 협력업체
					// substr(obj.Components[i].ID, 0, 8) == "div_prjt" ||	// 프로젝트
					// substr(obj.Components[i].ID, 0, 8) == "div_sale" ||	// SO
					// substr(obj.Components[i].ID, 0, 8) == "div_frbu" ||	//
					// substr(obj.Components[i].ID, 0, 8) == "div_tobu" ||	//
					// substr(obj.Components[i].ID, 0, 8) == "div_frli" ||	//
					// substr(obj.Components[i].ID, 0, 8) == "div_toli" ||	//
					// substr(obj.Components[i].ID, 0, 8) == "div_frsm" ||	//
					// substr(obj.Components[i].ID, 0, 8) == "div_mana" ||	// 영업담당자
					// substr(obj.Components[i].ID, 0, 8) == "div_plan" ||	// 년월
					// substr(obj.Components[i].ID, 0, 8) == "div_tosm"		//
					 substr(obj.Components[i].ID, 0, 4) == "div_"		//
				)
			{
				// 리사이즈 제외 
				bProcSizeFlag = false;
		...
		{
			//trace(" @@@ userdata 2: "+obj.Components[i].userdata);
			// form안에 컴포넌트 안에 컴포넌트처리
			if ( obj.Components[i].userdata != 'fix')
			{
				bProcSizeFlag = true;
				bX = true;
				if ( obj.Components[i].userdata != 'fix2' )
				{
					// y좌표 고정
					bY = true;
				}
		...
		else if ( obj.Components[i].GetType() == "Tab" ) 
		{
			//---------------------------------------------------------------
			// GRY : 추가 : 2012.09.03
			if (substr(obj.Components[i].ID, 0, 8) == "tab_hdel")
			{
				// 리사이즈 제외 
				bProcSizeFlag = false;
			}
...
//		trace(obj.Components[i].ID + "{" + obj.Components[i].GetType() + "} : " + obj.Components[i].Width);
		...
		if ( obj.Components[i].IsComposite() )
		{
			Gfn_SubFormResizeProcSD(obj.Components[i]);
		}
	}
...
function Gfn_frm_OnSizeSD(obj,nCx,nCy,nState)
{
