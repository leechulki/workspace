	...
	for ( var i = 0 ; i < this.Components.Count ; i++ )
	{
		...
		else if (this.Components[i].GetType() == "Div")
		{
			...
			else
			{
				...
				if ( this.Components[i].ID <> "div_code" )
				{
					...
					else if (this.Components[i].ID == "div_top")			// (상단)높이 고정. 2015.01.09
					{
						bX = true;
					}
					else if (this.Components[i].ID == "div_bottom")			// (하단)높이 변경. 2015.01.09
					{
						bY = true;
					}
					else
		...
		else if ( this.Components[i].GetType() == "Tab" ) 
		{
			...
			if (substr(this.Components[i].ID, 0, 8) == "tab_hdel" ||
				substr(this.Components[i].ID) == "tab_template")
			{
				// 리사이즈 제외 
				bProcSizeFlag = false;
			}
