/**
 * 파 일 명 : TransForTEAMESEVAL.java
 * 설    명 : 협력사평가 협력도평가 설치팀장별 처리(ES)
 * 작 성 자 : swn
...
public class TransForTEAMESEVAL
{
    ...
    * 기  타 :
    */
   public int ProcTransForCOLLTEAMESEVAL (String mandt,String scrGubun, Connection conn) throws SQLException, Exception
   {
      SqlExecutor db = null;
     ...
     for(int i=0; i<returnDsMonths.getRowCount(); i++) {
        ...
        // System.out.println("DIFFMONTHS -> "+i+", DIFFMONTHS--> "+ returnDsMonths.getColumnAsString(i, "DIFFMONTHS").toString().substring(0,8));
     }
     String VAR1 = "";
     String VAR2 = "";
     String VAR3 = "";
     ...
     for(int i=0; i<returnDsVar.getRowCount(); i++) {
      ...
      masterTeam.addParamObject("CHK_GBN", "11");
      masterTeam.addParamObject("CHK_GBN2", "02");
      ...
      if(returnDsTeam.getRowCount() != 0){
          System.out.println("==============================================================================");
          System.out.println("협력사평가 협력도평가 팀장별 처리 BATCH는 반기에 한번 실행됩니다.협력사평가 협력도평가 팀장별 처리 BATCH가 이미 실행되었습니다.");
          System.out.println("==============================================================================\n");
          //return 0;
      }

      years = YYYYMMDD0.substring(0,4);
    //  delProcForZPSTW1701 ( mandt, conn, "");
     ...
     sqlBuff1.append("  ,  '11'    --설치구분 CHAR    1   NOT NULL    PK \n");
...
//          sqlBuff.append("    AND LIFNO = ?       \n"); 
   ...
   public TransForTEAMESEVAL(Logger pLogger)
   {
      logger = pLogger;
   }
}