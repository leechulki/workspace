...
import hdel.sd.sbp.service.Sbp0060S;
import hdel.sd.sbp.domain.ZSDS0040;
...
@Controller
@RequestMapping("Sbp0060")
public class Sbp0060C {
	...
	public View test(MipParameterVO paramVO, Model model) {
		...
		ZSDS0040[] 	list 	= new ZSDS0040[]{};  // 프로젝트별 수금관리목록 WFC output list
		...
		try { 
			...
			// RFC CALL
			SapFunction stub = CallSAP.callSap(paramVO.getVariable("G_SYSID"), paramVO.getVariable("G_MANDT"), "hdel.sd.sbp.domain.ZWEB_SD_CHN_109R", obj, false);	

			// RFC 출력구조체로 out dataset 생성
			ds = CallSAP.isJCO() ? new Dataset() : ZSDS0040.getDataset();
			ds.setId("ds_list");  
	...
	public View update(MipParameterVO paramVO, Model model) {
		...
		// 입력 파라메터 처리 
		ZSDS0040[] list =  (ZSDS0040[])moveDs2Obj2(ds_list, ZSDS0040.class,"");
...
//		}
		...
		try { 
			...
			// RFC CALL
			SapFunction stub = CallSAP.callSap(paramVO.getVariable("G_SYSID"), paramVO.getVariable("G_MANDT"), "hdel.sd.sbp.domain.ZWEB_SD_CHN_109R_SAVE", obj, false);	
