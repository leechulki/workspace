...
import hdel.sd.ses.service.Ses0150S;
...
@Controller
@RequestMapping("ses0150")
public class Ses0150C {
	...
	@Autowired
	private Ses0150S service;
	
	@RequestMapping(value="header")
	public View Ses0150FindView(MipParameterVO paramVO, Model model) {
		...
		{ 
			...
			param.setInco (DatasetUtility.getString(dsCond,"INCO"        ));

			List<Ses0150> list = service.searchHeader(param);   			// 서비스CALL
			dsList.deleteAll();
			...
			for (int iRow=0;iRow<list.size();iRow++) {
				...
				for (int iCol=0;iCol<dsList.getColumnCount();iCol++)	{
					...
					else if (dsList.getColumnID(iCol).equals("ZRSRLT"))               dsList.setColumn(iRow, iCol, list.get(iRow).getZrsrlt()); 
	...
	public View Ses0150DtlView(MipParameterVO paramVO, Model model) {
		...
		{ 
			...
			List<Ses0150> list = service.searchDetail(param);  	// 서비스CALL
			dsList.deleteAll();
	...
	public View Ses0150ReceiptView(MipParameterVO paramVO, Model model)		{ 
	...
	public View Ses0150ReceiptCancelView(MipParameterVO paramVO, Model model) { 
