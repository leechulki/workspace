...
import hdel.sd.scl.domain.ZSDS0029;
...
@Controller
@RequestMapping("scl0010")
public class Scl0010C {
	...
	public View main(MipParameterVO paramVO, Model model) {
		...
		ZSDS0029[] 	list 	  = new ZSDS0029[]{};  // 프로젝트별 수금관리목록 RFC output list
		...
		try {
			...
			SapFunction stub = CallSAP.callSap(paramVO.getVariable("G_SYSID"), paramVO.getVariable("G_MANDT"), "hdel.sd.scl.domain.ZWEB_SD_CHN_123", obj, false);	
			// RFC 출력구조체로 out dataset 생성
			ds = CallSAP.isJCO() ? new Dataset() : ZSDS0029.getDataset();
			ds.setId("ds_list");  
