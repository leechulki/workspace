...
import hdel.sd.sch.service.Sch0063S;
...
@Controller
@RequestMapping("sch0063")
public class Sch0063C {
	...
	public View main(MipParameterVO paramVO, Model model) {
		...
		List<Sch0063> listSch0063 = null;

		Sch0063ParamVO param = new Sch0063ParamVO();
		try {
			param.setMANDT(paramVO.getVariable("G_MANDT"));
			param.setKIDNO(dsCond.getColumnAsString(0,"KIDNO"));
			param.setKUNNR(dsCond.getColumnAsString(0,"KUNNR"));
			
			listSch0063 = service.selectZFIT1015(param);

			dsList.deleteAll();

			CallSAPHdel.moveListToDs(dsList, Sch0063.class, listSch0063);
	...
	public View save(MipParameterVO paramVO, Model model) {
		...
		Sch0063ParamVO param = new Sch0063ParamVO();
		try {
			service.mergeZFIT1015(paramVO, model, session);
	...
	public View delete(MipParameterVO paramVO, Model model) {
		...
		Sch0063ParamVO param = new Sch0063ParamVO();
		try {
			service.deleteZFIT1015(paramVO, model, session);
